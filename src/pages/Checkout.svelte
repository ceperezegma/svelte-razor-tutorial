<script>
    ...
    onMount(() => {
        ...
        // if the user exits use stripe to fill the <div>
        if($cartTotal > 0){
            stripe = Stripe('pk_test_51J0AorKKHhjTEfHdkFLUAnNaEGj6JxxZzExOHPFH8LfaBhs5gZ1LrdtjzdkmkNKC9fAR16cNMii4RAyPnJxRRQ0O00wpaw6Gtu');
    ...
    async function handleSubmit(){
        ...
        if(token) {
            ...
            if(order){
                ...
                // delete the cat content
                cart.set([]);  
                // clean the cart variable in the local storage
...
{#if $cartTotal > 0}
    <section class="form">
        ...
        <div class="stripe-input">
            <!-- info -->
            <label for="" class="card-element">Credit or Debit Card</label>
            <p class="stripe-info">
            ...
            <div class="card-errors" bind:this={cardErrors} role="alert"></div>
        </div>
        <!-- end of stripe stuff -->
        <!-- error message -->
        {#if isEmpty}
